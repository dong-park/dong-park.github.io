...
- 빨리가는 유일한 방법은 언제나 코드를 깨끗하게 유지하는 습관이다. p7
- 단순한 코드 규칙으로 구현을 시작하는법 p13
    - 모든 테스트를 통과한다
    - 중복이 없다
    - 시스템 내 모든 설계 아이디어를 표현한다
    - 클래스, 메서드, 함수 등을 최대한 줄인다.
- 캠프장을 처음 왔을 때 보다 더 깨끗하게 하고 떠나라 p19
...
대다수 주석이 속하는 범주, 허술한 코드를 지탱하거나, 엉뚱한 코드를 변명하거나, 미숙한 결정을 합리화 한다.
